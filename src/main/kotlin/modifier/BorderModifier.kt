...
data class Border(val strokeWidth: Dp, val color: Color) {
    companion object {
        fun of(strokeWidth: Dp, color: Color): Border? =
            if (strokeWidth == 0.dp) null else Border(strokeWidth, color)
    }
}
...
fun Modifier.border(
    ...
    border(
        start = Border.of(start, color),
        top = Border.of(top, color),
        end = Border.of(end, color),
        bottom = Border.of(bottom, color)
    )
...
private fun DrawScope.drawEndBorder(
    ...
    if (border.strokeWidth == 0.dp) return
